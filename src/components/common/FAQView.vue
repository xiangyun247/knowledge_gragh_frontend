<template>
  <div class="faq-page">
    ...
    <div class="page-header">
      ...
      <p class="page-subtitle">了解认知健康管理系统的使用方法、知识图谱构建、历史记录等常见问题</p>
    </div>
  ...
  methods: {
    ...
    generateMockData() {
      ...
      this.faqCategories = [
        ...
        { id: 'education', name: '患者教育中心', icon: 'el-icon-document' },
      ...
      this.faqs = [
        {
          id: 'faq_001',
          category: 'basic',
          question: '什么是忆路康认知健康管理系统？',
          answer: '忆路康认知健康管理系统是面向认知障碍老年人的智能评估平台：通过 EEG 脑电信号、NASA-TLX 主观问卷和行为埋点三种模态数据融合，量化评估老年人的认知负荷水平。系统同时提供知识图谱可视化、智能对话辅助、患者教育、服药提醒等功能，为照护者和医疗机构提供全面的数据支持。',
        ...
        {
          id: 'faq_auth_permissions',
          category: 'basic',
          question: '未登录用户可以做什么？哪些操作必须登录？',
          answer: '未登录状态下，您可以浏览封面页和首页、查看知识图谱可视化、进行实体搜索、查看知识库与常见问题等页面内容。为了保护数据与个人记录，以下操作必须登录后才能使用：1）发送聊天问答（包括语音输入、图片解读）；2）生成和保存患者教育内容；3）上传文件、触发图谱构建或文档入库；4）查看完整的历史记录和「我的患者教育」。如果在未登录时尝试执行这些操作，系统会提示先登录并自动跳转到登录页。',
          related: ['faq_002', 'faq_004', 'faq_edu_overview']
        },
        {
          id: 'faq_004',
          category: 'chat',
        ...
        },
        {
          id: 'faq_chat_models',
          category: 'chat',
          question: '聊天问答支持哪些大模型？如何切换？',
          answer: '系统在后端通过统一的 llm/models_config.py 适配了包括 DeepSeek、OpenAI GPT、Gemini、通义千问、豆包、智谱 GLM、文心一言、Kimi、Grok 等多种主流大模型。前端聊天页面输入框下方提供「模型」下拉框：仅会显示已在后端成功配置的模型，未配置的模型会带有「(未配置)」提示并在选择时弹出说明。一般建议：默认使用 DeepSeek（综合效果和速度较平衡）；如需更强推理能力可选择高阶模型；如关注成本或速度可选择轻量模型。切换模型后，新问题会使用所选模型回答，已有对话内容会保留，方便在不同模型之间对比效果。',
          related: ['faq_004', 'faq_005']
        },
        {
          id: 'faq_chat_multimodal',
          category: 'chat',
          question: '问答页面支持语音输入和图片解读吗？',
          answer: '是的。聊天输入框右侧提供语音按钮和「图片解读」入口：1）点击麦克风图标即可开始录音，再次点击结束，系统会调用本地 Whisper 模型进行中文语音转文字，识别结果会自动填入输入框；2）通过图片入口上传检查报告截图、化验单或简单影像图，系统会先进行 OCR 文本识别，再结合模型生成通俗的图像解读。语音和图片功能主要用于辅助输入与理解：不建议上传包含个人隐私的照片；对模糊、噪声较大的录音和图片，识别可能不准确，应以医生面诊和正式报告为准。',
          related: ['faq_004', 'faq_005', 'faq_006']
        },
        {
          id: 'faq_005',
          category: 'chat',
        ...
        },
        {
          ...
          answer: '知识图谱可视化功能可展示 30 种医疗实体（如疾病、症状、药物、实验室检查、解剖部位、科室等）及 20 种关系类型（如症状或体征、用于治疗、并发症、发病部位等）之间的关系网络。您可查看实体详细属性、关联关系类型与强度。系统支持缩放、拖拽、按类型筛选、全屏等交互。图谱按实体类型使用不同颜色标识，图例显示各类含义，便于理解知识结构。',
        ...
        },
        {
          ...
          answer: '在图谱可视化页面顶部工具栏中，可使用搜索框输入实体名称，系统会自动定位并高亮。筛选下拉支持按全部 30 种实体类型筛选（如疾病、症状、药物、实验室检查、解剖部位、科室等）。点击图例按钮可显示或隐藏图例。使用「返回」按钮可清空搜索与筛选、恢复完整视图。',
        ...
        },
        {
          ...
          category: 'search',
          question: '实体搜索支持哪些类型？',
          answer: '实体搜索支持全部 28 种医疗实体类型，如疾病、症状、药物、实验室检查、解剖部位、科室、预后等。您可在搜索框上方选择具体类型或「全部类型」，也可直接输入实体名称搜索。系统支持模糊匹配，输入不完整也能找到相关实体。搜索结果会显示实体名称、类型标签和简要描述。',
        ...
        },
        {
          ...
          answer: '系统支持上传 TXT、PDF、JSON、CSV 等格式的数据文件。上传后可选择「入库」加入知识库，或「生成知识图谱」构建/更新图谱。系统支持 30 种实体类型和 20 种关系类型，页面提供数据模板下载（疾病 JSON、症状 CSV、药物 JSON、关系 XML），便于按规范准备数据。批量构建功能可对已上传文件一次性触发知识图谱构建。',
        ...
        },
        {
          id: 'faq_edu_overview',
          category: 'education',
          question: '患者教育中心是什么？可以做什么？',
          answer: '患者教育中心是专门为临床医生和科普运营设计的模块，可一键生成面向患者的「疾病说明书」。您只需输入一个主题（例如「轻度认知障碍老人居家注意事项」），系统会基于知识图谱和文献自动生成结构化的患者教育内容，包括标题、分节小结（病情简介、注意事项、饮食建议、复查与随访等）以及结尾的温馨提示，方便直接打印或发送给患者。',
          related: ['faq_edu_generate', 'faq_edu_save']
        },
        {
          id: 'faq_edu_generate',
          category: 'education',
          question: '如何在患者教育中心生成一份患者教育？',
          answer: '在「患者教育中心」左侧输入框中，输入您关心的主题（例如具体疾病、术后注意事项等），可勾选是否为每个小节生成插图，然后点击「生成患者教育」。系统会调用后端的专用 Agent（intent=patient_education），自动输出结构化的小节内容，并在中间区域以卡片形式预览。生成完成后，您可以继续编辑主题，再次生成新的版本。',
          related: ['faq_edu_overview', 'faq_edu_save']
        },
        {
          id: 'faq_edu_save',
          category: 'education',
          question: '生成的患者教育可以保存和复用吗？',
          answer: '可以。在生成结果下方点击「保存到我的患者教育」，系统会把当前的结构化内容和可选配图一起保存到个人空间。右侧「我的患者教育」列表会显示已保存的记录，支持点击查看详情并重新加载为当前预览；列表同时展示主题、创建时间以及是否包含配图，方便快速检索与复用。',
          related: ['faq_edu_overview', 'faq_edu_images']
        },
        {
          id: 'faq_edu_images',
          category: 'education',
          question: '患者教育中心中的插图是如何生成的？',
          answer: '当勾选「为每个小节生成一张插图」时，系统会调用后端的图像生成接口（如 GLM-Image），根据每个小节的标题和内容生成一张配图，并在右侧预览中与对应小节并排展示。生成失败时文字内容仍然可用，系统会给出「文字已生成，配图生成失败或暂不可用」的提示。插图信息会和患者教育一起保存，可在之后打印或导出时一并展示。',
          related: ['faq_edu_overview', 'faq_edu_save']
        },
        {
          id: 'faq_edu_disclaimer',
          category: 'education',
          question: '患者教育内容是否可以直接用于临床决策？',
          answer: '患者教育内容仅用于健康宣教和医患沟通辅助，不能替代医生的专业诊疗建议或正式书面报告。系统生成的文字和插图基于知识图谱与公开文献综合而来，可能存在不完整或不适用于特定个体的情况。实际诊断、用药和治疗方案应始终由具有资质的医生结合患者具体病情、检查结果和相关指南独立判断。',
          related: ['faq_edu_overview', 'faq_016']
        }
      ];
    },
