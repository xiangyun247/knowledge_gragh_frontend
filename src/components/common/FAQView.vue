<template>
  <div class="faq-page">
    ...
    <div class="page-header">
      ...
      <p class="page-subtitle">了解认知负荷评估系统的使用方法、知识图谱构建、历史记录等常见问题</p>
    </div>
      ...
      this.faqs = [
        {
          id: 'faq_001',
          category: 'basic',
          question: '什么是忆路康认知负荷评估系统？',
          answer: '忆路康认知负荷评估系统是面向认知障碍老年人的智能评估平台：通过 EEG 脑电信号、NASA-TLX 主观问卷和行为埋点三种模态数据融合，量化评估老年人的认知负荷水平。系统同时提供知识图谱可视化、智能对话辅助、患者教育、服药提醒等功能，为照护者和医疗机构提供全面的数据支持。',
        ...
        },
        {
          ...
          answer: '知识图谱可视化功能可展示 28 种医疗实体（如疾病、症状、药物、实验室检查、解剖部位、科室等）及 21 种关系类型（如症状或体征、用于治疗、并发症、发病部位等）之间的关系网络。您可查看实体详细属性、关联关系类型与强度。系统支持缩放、拖拽、按类型筛选、全屏等交互。图谱按实体类型使用不同颜色标识，图例显示各类含义，便于理解知识结构。',
        ...
        },
        {
          ...
          answer: '在图谱可视化页面顶部工具栏中，可使用搜索框输入实体名称，系统会自动定位并高亮。筛选下拉支持按全部 28 种实体类型筛选（如疾病、症状、药物、实验室检查、解剖部位、科室等）。点击图例按钮可显示或隐藏图例。使用「返回」按钮可清空搜索与筛选、恢复完整视图。',
        ...
        },
        {
          ...
          answer: '系统支持上传 TXT、PDF、JSON、CSV 等格式的数据文件。上传后可选择「入库」加入知识库，或「生成知识图谱」构建/更新图谱。系统支持 28 种实体类型和 21 种关系类型，页面提供数据模板下载（疾病 JSON、症状 CSV、药物 JSON、关系 XML），便于按规范准备数据。批量构建功能可对已上传文件一次性触发知识图谱构建。',
